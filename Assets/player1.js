...
private var in_play: List.<GameObject>;
public var deck: List.<GameObject>;

private var max_mana: int[] = [0, 0, 0, 0];
private var current_mana: int[] = [0, 0, 0, 0];

private var chosen_mana: boolean = false;

function Start () 
{
	GameObject.FindGameObjectWithTag("game_master").GetComponent("game_master").Check_In_Player_1(gameObject);
	...
	if(hand.Count < 10)
	{
		for(var i: int = 0; i < amount; i++)
		{
			if(deck.Count > 0)
			{
				...
				var material_card: GameObject = Network.Instantiate(Resources.Load("networked_card"), Vector3(0, 0, 0), Quaternion.identity, 0);
